...
contract KittyShelter {
    ...
    // uint[] catIDS;
    mapping (uint => uint) public catIDS;
    KittenRegistry public registry;
    function setKittenRegistry(address _registry) public  payable {
        require(msg.sender == owner);
        registry = KittenRegistry(_registry);
    }
    
    constructor(KittenRegistry _registry) public payable{
        registry = _registry;
    }
    function getTime () public view returns(uint256 time){
        return block.timestamp - 30 days;
    }
    function storeKitty(uint256 catId, uint256 time) public payable  {
        catIDS[catId] = getTime() + time;
        registry.transferToshelter(msg.sender,address(this),catId);
    }
    function retrieveKitty(uint256 catId) public{
        // if (msg.sender == registry.ownerOf(catId)){
        require(getTime() >= catIDS[catId],"It's not time YET");
        require(msg.sender == registry.ownerOf(catId),"Not your cat");
        registry.transferToshelter(address(this),msg.sender,catId);
        // }
    }
}
...
contract KittenRegistry {
    ...
    function transferFrom(address from, address to, uint256 catId) public {
        address owner = ownerOf(catId);
        // require(msg.sender == owner || getApproved(catId) == msg.sender); The constructor should be payable if you send value
    ...
    function transferToshelter(address from, address to, uint256 catId) public {
        address owner = ownerOf(catId);
        // require(msg.sender == owner || getApproved(catId) == msg.sender); The constructor should be payable if you send value
